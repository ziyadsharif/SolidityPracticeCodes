...
contract BlindBidding {
    ...
    constructor(uint _biddingTime, uint _revealTime, address payable _payee) public {
        payee = _payee;
        biddingEnd = now + _biddingTime;
        divulgeEnd = biddingEnd + _revealTime;
    }

    function bid(bytes32 _blindedBid) public payable Before(biddingEnd){
    ...
    function reveal(uint[] memory _values, bool[] memory _fake, bytes32[] memory _secret) public After(biddingEnd) Before(divulgeEnd) {
    ...
    function auctionEnd() public After(divulgeEnd){
        require(!ended);
